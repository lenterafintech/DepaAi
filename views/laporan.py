"""Hasil yang Anda Jalankan: satu hasil terakhir per kategori metode, otomatis.

Sebelumnya halaman ini hanya berisi apa yang disimpan manual lewat tombol "Simpan
ke laporan". Sejak ``ui.catat_hasil`` dipanggil otomatis begitu sebuah metode
berhasil dihitung (lihat ``nalardata/ui.py``), isi di sini selalu sinkron dengan
metode yang benar-benar dijalankan pengguna di tab Analisis — tanpa tombol apa pun.
"""
...
def render() -> None:
    if not ui.butuh_fitur("keranjang"):
        return
    ...
    if isi.kosong():
        ui.keadaan_kosong(
            ...
            "Jalankan metode apa pun di tab Analisis — Regresi, CFA, Uji Non-parametrik, "
            "dan lainnya. Hasilnya akan muncul di sini secara otomatis.",
            ikon="＋",
        )
        return
    ...
    if not ui.paket_aktif().punya("unduh_laporan"):
        ...
        if kode in {"py", "r"}:
            kanan.info(
                ...
                "Ringkasan Otomatis — bukan dari keranjang ini.",
                icon=":material/info:",
            )
        else:
            try:
